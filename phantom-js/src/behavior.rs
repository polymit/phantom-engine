...
impl BehaviorEngine {
    ...
    pub fn generate_mouse_path(&self, from: (f64, f64), to: (f64, f64)) -> Vec<(f64, f64)> {
        ...
        let n = 20 + (rng.random::<u8>() % 20) as usize;
    ...
    pub fn click_hesitation_ms(&self) -> u64 {
        let sample = self.click_hesitation.sample(&mut rand::rng());
