...
async fn eval_float(s: &Tier1Session, code: &str) -> f64 {
    ...
    result.parse::<f64>().expect("result must be parseable as f64")
}
...
async fn session_with_shims() -> Tier1Session {
    ...
    let start = shims_source.find("// 14. Font measureText").expect("Could not find start of Shim 14");
    let end = shims_source.find("// 15. WebRTC IP leak prevention").expect("Could not find start of Shim 15");
...
async fn font_shim_arial_width_is_correct() {
    ...
    let width = eval_float(&s, "
        (function() {
            var ctx = { font: '16px Arial' };
            return CanvasRenderingContext2D.prototype.measureText.call(ctx, 'x').width;
        })()
    ").await;
    assert!((56.28..=56.32).contains(&width), "Arial at 16px must be ~56.30 (+-0.02), got {}", width);
    s.destroy();
}
...
async fn font_shim_verdana_width_is_correct() {
    ...
    ").await;
    assert!((61.38..=61.42).contains(&width), "Verdana at 16px must be ~61.40, got {}", width);
    s.destroy();
}
...
async fn font_shim_trebuchet_width_is_correct() {
    ...
    ").await;
    assert!((58.08..=58.12).contains(&width), "Trebuchet MS at 16px must be ~58.10, got {}", width);
    s.destroy();
}
...
async fn font_shim_unknown_font_returns_number() {
    ...
    ").await;
    assert_eq!(t, "number", "unknown font must still return a number — not throw");
    s.destroy();
}
...
async fn font_shim_document_fonts_check_known() {
    let s = session_with_shims().await;
    let result = eval(&s, "String(document.fonts && document.fonts.check('16px Arial'))").await;
    assert_eq!(result, "true", "RISK-25: document.fonts.check must return true for known font");
    s.destroy();
}
...
async fn font_shim_document_fonts_load_known() {
    ...
    assert_eq!(result, "function", "RISK-25: document.fonts.load must return a Promise for known font");
    s.destroy();
}
