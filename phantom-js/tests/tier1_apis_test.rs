...
async fn test_set_timeout_fires() {
    ...
    local.run_until(async move {
        ...
        session.eval("globalThis.__timer_fired = false;").await.unwrap();
        ...
        session.eval("setTimeout(function() { globalThis.__timer_fired = true; }, 50);")
            .await.unwrap();
        ...
        assert_eq!(fired, "true", "setTimeout callback must have fired after 50ms delay");
...
async fn test_mutation_bridge_dispatches() {
    ...
    bridge.notify_mutation(r#"{"type":"childList","target":"n_1"}"#).await;

    // Verify the mutation was received
    let received = session.eval("globalThis.__mutation_received").await.unwrap();
...
async fn test_session_startup_time() {
    ...
    let session = phantom_js::tier1::session::Tier1Session::new().await.unwrap();
