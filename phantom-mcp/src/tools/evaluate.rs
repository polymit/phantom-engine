    ...
    let raw = match session.eval(&p.script).await {
        ...
        Err(e) => {
            ...
            let (code, destroy_session) = if timed_out {
                ...
                // ARCHITECTURAL NOTE: QuickJS timeouts and OOMs are isolated by the Tier1Pool.
                // The dirty runtime is discarded by the pool automatically upon error.
                // We do NOT destroy the overall Session, allowing the agent to recover and retry.
                ("js_timeout", false)
