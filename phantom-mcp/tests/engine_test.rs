...
async fn scene_graph_before_navigate_returns_no_page_error() {
    ...
    assert!(err_str.contains("no_page_loaded"),
        "error code must be no_page_loaded, got: {}", err_str);
...
async fn scene_graph_after_store_returns_valid_cct() {
    ...
    assert!(cct.starts_with("##PAGE"),
        "CCT must start with ##PAGE");
    assert!(cct.contains("test.local"),
        "CCT must contain the URL");

    let node_count = cct.lines().filter(|l| !l.starts_with("##")).count();
    assert!(node_count > 0,
        "CCT must have at least one node");

    println!("scene_graph after store: nodes={}, cct_len={}",
             node_count, cct.len());
}
...
async fn scene_graph_selective_mode_accepted() {
    ...
    adapter.store_page(SessionPage::new(
        page, "https://login.test".to_string(), 200
    ));
    ...
    assert!(resp.error.is_none(),
        "scene_graph selective must not error: {:?}", resp.error);
    let result = resp.result.unwrap();
    assert!(result["cct"].as_str().is_some(),
        "result must have cct field");
    assert!(result["node_count"].as_u64().unwrap_or(0) > 0,
        "node_count must be > 0");
    println!("selective mode: node_count={}, mode={}",
...
async fn scene_graph_scroll_params_accepted() {
    ...
    let page = process_html(
        ...
        "https://scroll.test", 1280.0, 720.0,
    ).unwrap();
    adapter.store_page(SessionPage::new(
        page, "https://scroll.test".to_string(), 200
    ));
    ...
    let result = resp.result.unwrap();
    assert!(result["cct"].as_str().unwrap_or("").starts_with("##PAGE"),
...
async fn scene_graph_cct_header_contains_correct_url() {
    ...
    assert!(header.contains("specific-url.test"),
        "CCT header must contain the exact URL, got: {}", header);
