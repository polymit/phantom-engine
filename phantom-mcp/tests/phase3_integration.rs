...
use phantom_mcp::engine::{SessionPage, SendablePage};
...
async fn phase3_full_pipeline_navigate_and_scene_graph() {
    ...
    let req = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":1,"method":"browser_get_scene_graph","params":{}}"#).unwrap();
...
async fn phase3_click_login_button() {
    let (adapter, server) = setup_with_page().await;
    let req = McpServer::parse_request(r##"{"jsonrpc":"2.0","id":1,"method":"browser_click","params":{"selector":"#login-btn"}}"##).unwrap();
    let resp = server.handle_request(&adapter, req, None).await.unwrap();
    ...
    assert_eq!(result.get("selector").unwrap().as_str().unwrap(), "#login-btn");
}
...
async fn phase3_press_tab_to_advance_focus() {
    let (adapter, server) = setup_with_page().await;
    let req = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":1,"method":"browser_press_key","params":{"key":"Tab"}}"#).unwrap();
    let resp = server.handle_request(&adapter, req, None).await.unwrap();
...
async fn phase3_cookies_workflow() {
    let (adapter, server) = setup_with_page().await;
    let req = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":1,"method":"browser_get_cookies","params":{}}"#).unwrap();
    let resp = server.handle_request(&adapter, req, None).await.unwrap();
    assert_eq!(resp.result.unwrap().get("cookies").unwrap().as_array().unwrap().len(), 0);
    // 2
    let req2 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":2,"method":"browser_clear_cookies","params":{}}"#).unwrap();
    let resp2 = server.handle_request(&adapter, req2, None).await.unwrap();
    assert_eq!(resp2.result.unwrap().get("cleared").unwrap().as_bool(), Some(true));
    // 3
    let req3 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":3,"method":"browser_get_cookies","params":{}}"#).unwrap();
    let resp3 = server.handle_request(&adapter, req3, None).await.unwrap();
    assert_eq!(resp3.result.unwrap().get("cookies").unwrap().as_array().unwrap().len(), 0);
}

#[tokio::test]
async fn phase3_snapshot_and_verify() {
    let (adapter, server) = setup_with_page().await;
    let req = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":1,"method":"browser_session_snapshot","params":{}}"#).unwrap();
    let resp = server.handle_request(&adapter, req, None).await.unwrap();
...
async fn phase3_tab_full_lifecycle() {
    ...
    let id1 = resp1.result.unwrap().get("tab_id").unwrap().as_str().unwrap().to_string();
    
    // new_tab
    let req2 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":2,"method":"browser_new_tab","params":{"url":"https://tab2.test"}}"#).unwrap();
    let resp2 = server.handle_request(&adapter, req2, None).await.unwrap();
    let id2 = resp2.result.unwrap().get("tab_id").unwrap().as_str().unwrap().to_string();
    
    // list_tabs
    let req3 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":3,"method":"browser_list_tabs","params":{}}"#).unwrap();
    let resp3 = server.handle_request(&adapter, req3, None).await.unwrap();
    let tabs = resp3.result.unwrap().get("tabs").unwrap().as_array().unwrap().clone();
    assert_eq!(tabs.len(), 2);
    
    // switch_tab
    let req4 = McpServer::parse_request(&format!(r#"{{"jsonrpc":"2.0","id":4,"method":"browser_switch_tab","params":{{"tab_id":"{}"}}}}"#, id1)).unwrap();
    server.handle_request(&adapter, req4, None).await.unwrap();
    
    // list active
    let req5 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":5,"method":"browser_list_tabs","params":{}}"#).unwrap();
    let resp5 = server.handle_request(&adapter, req5, None).await.unwrap();
    let tabs_after = resp5.result.unwrap().get("tabs").unwrap().as_array().unwrap().clone();
    let active_tab = tabs_after.iter().find(|t| t.get("active").unwrap().as_bool().unwrap()).unwrap();
    assert_eq!(active_tab.get("id").unwrap().as_str().unwrap(), id1);
    
    // close
    let req6 = McpServer::parse_request(&format!(r#"{{"jsonrpc":"2.0","id":6,"method":"browser_close_tab","params":{{"tab_id":"{}"}}}}"#, id2)).unwrap();
    server.handle_request(&adapter, req6, None).await.unwrap();
    
    // list remaining
    let req7 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":7,"method":"browser_list_tabs","params":{}}"#).unwrap();
    let resp7 = server.handle_request(&adapter, req7, None).await.unwrap();
    assert_eq!(resp7.result.unwrap().get("tabs").unwrap().as_array().unwrap().len(), 1);
}
...
async fn phase3_sse_subscribes_and_receives_delta() {
    ...
    let msg = timeout(Duration::from_millis(100), rx.recv()).await.unwrap().unwrap();
...
async fn phase3_all_mcp_tools_are_registered() {
    ...
    let server = McpServer::new_with_adapter(None, adapter.clone());
    let tools = vec![
        "browser_navigate", "browser_get_scene_graph", "browser_click",
        "browser_type", "browser_press_key", "browser_evaluate",
        "browser_new_tab", "browser_switch_tab", "browser_list_tabs",
        "browser_close_tab", "browser_get_cookies", "browser_set_cookie",
        "browser_clear_cookies", "browser_session_snapshot"
    ];
    for t in tools {
        let req = McpServer::parse_request(&format!(r#"{{"jsonrpc":"2.0","id":1,"method":"{}","params":{{}}}}"#, t)).unwrap();
        let resp = server.handle_request(&adapter, req, None).await.unwrap();
        if let Some(err) = resp.error {
...
async fn phase3_real_page_navigation() {
    ...
    if let Ok(resp) = server.handle_request(&adapter, req, None).await {
        if resp.error.is_none() {
            let req2 = McpServer::parse_request(r#"{"jsonrpc":"2.0","id":2,"method":"browser_get_scene_graph","params":{}}"#).unwrap();
            let resp2 = server.handle_request(&adapter, req2, None).await.unwrap();
