...
fn process_node_ids(
    ...
    initial_tag: &str,
    initial_tag_idx: usize,
    map: &mut StableIdMap,
) {
    let mut stack = vec![(
        root_node_id,
        initial_parent_path.to_string(),
        initial_tag.to_string(),
        initial_tag_idx,
    )];

    while let Some((node_id, parent_path, tag, tag_idx)) = stack.pop() {
        ...
        // We push children in reverse order to maintain the same traversal order as recursion.
        let mut children: Vec<_> = node_id.children(&tree.arena).collect();
        children.reverse();

        let mut tag_indices: HashMap<String, usize> = HashMap::new();
        // Since we are pushing to stack in reverse, we need to pre-calculate the tag indices
