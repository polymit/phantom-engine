...
pub fn compute_relevance(node: &DomNode, semantic: &SemanticInfo, task_hint: &str) -> f32 {
    ...
    if !hint_lower.is_empty() {
        ...
        if hint_lower
            .split_whitespace()
            .any(|word| text_lower.contains(word) && word.len() > 2)
        {
            score += 0.3;
        }
        ...
        if hint_lower
            .split_whitespace()
            .any(|word| label_lower.contains(word) && word.len() > 2)
        {
            score += 0.3;
        }
    }

    let is_login_task = hint_lower.contains("login")
        || hint_lower.contains("sign in")
        || hint_lower.contains("password");
    if is_login_task {
